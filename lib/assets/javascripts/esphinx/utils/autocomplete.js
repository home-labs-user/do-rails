            ...
            var
                ...
                inlineSearch = function(element, options) {
                    ...
                    eventListenerMethod = function(e) {
                        cachedElement = cachedElement || element
                            .clone(true);
                        ...
                        if (searchTextBox.value()) {
                            ...
                            if (searchTextLength < cachedSearchText.length) {
                                cachedElement = element.clone(true);
                            }

                            cachedSearchText = searchTextBox.value();
                            resolvedList = resolveList(cachedElement,
                                options.order);
                            foundElements = resolvedList.elements;

                            if (foundElements.some()) {
                                resolvedElementsParent = cachedElement
                                    .clone();
                                cachedElement = resolvedElementsParent
                                    .append(resolvedList.elements);
                            }

                            callback.call($(this), resolvedList, e);
                        } else {
                            // reset
                            elements2Recovery = [];
                            cachedSearchText = "";
                                ...
                                .get({
                                    ...
                                    success: function(r) {
                                        ...
                                        resolvedList = resolveList(element,
                                            options.order);
                                        callback.call(searchTextBox, resolvedList,
                                            e);
                                        if (element.length) {
                ...
                // [
                    // {
                        ...
                        // researchedSlice: "...",
                        // mapping: [
                            // {wordIndex: ..., sliceIndex: ...},
                            // {...}
                        // ]
                    // },
                    // {...}
                // ]
                mapIf = function(searchTextAsArray, wordList) {
                    var
                        iterator,
                        sliceIndex,
                        regexp,
                    ...
                    if (hashSetOfWords.length >= searchTextAsArray.length) {
                ...
                resolveList = function(list, order) {
                    ...
                    var
                        ...
                        callback = function(element) {
                            ...
                            wordList = textNode.split(" ");
                            mapped = mapIf(textBoxContentArr, wordList);
                            if (mapped) {
                                mapped.element = element;
                                maps.push(mapped);
                            }
                        };

                    if (list.isA(window.Element)) {
                        list = list.clone(true);
                    }
