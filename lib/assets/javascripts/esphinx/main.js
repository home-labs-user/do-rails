var
    // mudar para alguma coisa a ver com API
    esPhinx,
    Iterable,
    Extensor,
    BFS,
...
(function ($module) {
    ...
    var
        ...
        selectInBreadth = function (iterable, callback) {
            var
                returned,
                strategy = Iterable.Factory.new(BFS.new(iterable)),
                selected = [];
    ...
    esPhinx.extend(true, {
        ...
        selectInBreadth: function(iterable, callback) {
            return selectInBreadth(iterable, callback);
        },

        select: function(iterable, callback) {
            var
                returned,
                current,
                selected = [],
                iterator = Iterable.new(iterable),
            ...
            if (count) {
                do {
                    ...
                    returned = callback.call(iterable, current,
                                             keys[iterator.index()]);
                    if (returned) {
        ...
        selectByText: function (text, context) {
            ...
            this.select(this(resolveContext(context)), function (element) {
                collection = collection.concat(document.evaluate(
                    "descendant-or-self::*[normalize-space(.)='" + text.trim() +
