    ...
    var
        areEquivalents = function(object1, object2) {
            ...
            if (object1 == object2) {
                ...
                // debugger
                if (object1 instanceof $ && !(object1 instanceof Function)) {
                    if (object1 instanceof window.Node) {
                        if (object1.isEqualNode(object2)) {
                            return true;
                        }
                    } else {
                        objectAttributes = $.attributes(object1);
                        comparatorAttributes = $.attributes(object2);
                        objectMethods = $.methods(object1);
                        comparatorMethods = $.methods(object2);

                        // if doesn't have attributes
                        if (!objectAttributes.length) {
                            if (objectMethods.length) {
                                if (objectMethods.length == comparatorMethods
                                        .length &&
                        ...
                        if (objectAttributes.length ==
                            ...
                            iterator = Iterable.Proxy.new(objectAttributes);

                            iterator.each(function(attribute) {
                                objectAttributeValue = object1[attribute];
                                comparatorAttributeValue = object2[attribute];

                                // areFromClass
                                if (typeof objectAttributeValue == "boolean" &&
                                ...
                                if (objectAttributeValue &&
                                    comparatorAttributeValue) {
                                    if ($.hasSameClass(objectAttributeValue,
                                        comparatorAttributeValue)) {
