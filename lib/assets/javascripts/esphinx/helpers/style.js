...
(function ($) {
    ...
    $.extend({
        ...
        prototype: {
            ...
            horizontalToggle: function (initialMargin) {
                var
                    self = this;
                ...
                self.each(function (node) {
                    ...
                    if (initialMargin) {
                        if (node.css("margin-left") == initialMargin
                            || node.css("margin-right") == initialMargin) {
                            node.css("margin-left", -(node.width()
                            + node.css("margin-left")) + "px");
                            node.css("margin-right", -(node.width()
                            + node.css("margin-right")) + "px");
                        } else {
                            node.css("margin-left", initialMargin + "px");
                            node.css("margin-right", initialMargin + "px");
                        }
                    } else {
                        if (node.css("margin-left") < 0
                            || node.css("margin-right") < 0) {
                            node.css("margin-left", "initial");
                            node.css("margin-right", "initial");
                        } else {
                            node.css("margin-left", -(node.width()
                            + node.css("margin-left")) + "px");
                            node.css("margin-right", -(node.width()
