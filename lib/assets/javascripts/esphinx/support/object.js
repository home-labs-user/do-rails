...
(function ($) {
    ...
    if (!Object["destroySingleton"]) {
        $.defineProperty($.prototype, "destroySingleton", {
            value: function () {
                var
                    self = this,
                    instances = [],
                ...
                Singleton.instances.forEach(function (v) {
                    instances.push(v.object);
                });

                if (arguments.length) {
                    index = parseInt(instances
                        .indexOfEquivalent(arguments[0]), 10);
                    Singleton.instances.destroy(self[index]);
                } else {
                    index = parseInt(instances.indexOfEquivalent(self), 10);
                    Singleton.instances.destroy(Singleton.instances[index]);
                }
