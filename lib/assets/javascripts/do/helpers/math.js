  ...
  _arr.map(function(item) {
    ...
    while ( dividend > 1 ) {
      ...
      if( dividend.firstMultiple() === (_aux[_aux.length -1]) ) {
        ...
        // map smaller factors
        powFactor = Math.pow( (_aux[_aux.length -1]).firstMultiple(), count );

        if( _aux[_aux.length -1] in smallerFactorsHash ) {
          if( powFactor < smallerFactorsHash[_aux[_aux.length -1]] ) {
            smallerFactorsHash[_aux[_aux.length -1]] = powFactor;
          }
        } else {
          smallerFactorsHash[_aux[_aux.length -1]] = powFactor;
        }
  ...
  _primes.map(function(n, i) {
    if( n === _primes[i+1] ) {
      ...
      if( count === _arr.length ) {
        smallerProductCommon *= smallerFactorsHash[n];
      }
      count = 1;
   }
