        ...
        _arr.map(function(item) {
            ...
            while (dividend > 1) {
                ...
                if( dividend.firstMultiple() === (_aux[_aux.length -1]) ) {
                    ...
                    //_assembledPrimes.push(_aux[_aux.length -1]);                    
                    _primes.push(_aux[_aux.length -1]);

                    // map                    
                    if (_aux[_aux.length -1] in primesCommon) {
                        currFactors = (_aux[_aux.length -1]).firstMultiple().toString() + '^' + count.toString();
                        if (currFactors < primesCommon[_aux[_aux.length -1]] ) {
                            primesCommon[_aux[_aux.length -1]] = currFactors;
                        }
                    } else {
                        primesCommon[_aux[_aux.length -1]] = (_aux[_aux.length -1]).firstMultiple().toString() + '^' + count.toString();
                    }
